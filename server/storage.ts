...
export interface IStorage {
  ...
  // Content backup and versioning methods
  getContentBackups(blogPostId: number): Promise<ContentBackup[]>;
...
export class MemStorage implements IStorage {
  ...
  async createBlogPost(insertPost: InsertBlogPost): Promise<BlogPost> {
    ...
    const post: BlogPost = {
      ...
      contentType: insertPost.contentType || "monthly",
      scheduledFor: insertPost.scheduledFor || null,
      autoGenerated: insertPost.autoGenerated || null,
      seoScore: insertPost.seoScore || null,
      socialMediaPosted: insertPost.socialMediaPosted || null,
    };
  ...
  // Stub implementations for new methods (MemStorage is not used in production)
  async getSeoData(): Promise<SeoData[]> { return []; }
  async getSeoDataByPage(page: string): Promise<SeoData | undefined> { return undefined; }
  async createSeoData(seoData: InsertSeoData): Promise<SeoData> { 
    return { id: 1, page: seoData.page, title: seoData.title, description: seoData.description, keywords: seoData.keywords ?? null, ogTitle: seoData.ogTitle ?? null, ogDescription: seoData.ogDescription ?? null, ogImage: seoData.ogImage ?? null, ogImageAlt: seoData.ogImageAlt ?? null, twitterTitle: seoData.twitterTitle ?? null, twitterDescription: seoData.twitterDescription ?? null, twitterImage: seoData.twitterImage ?? null, canonicalUrl: seoData.canonicalUrl ?? null, metaRobots: seoData.metaRobots ?? null, structuredData: seoData.structuredData ?? null, lastUpdated: new Date() }; 
  }
  async updateSeoData(page: string, seoData: Partial<InsertSeoData>): Promise<SeoData | undefined> { return undefined; }
  async getContentSchedules(): Promise<ContentSchedule[]> { return []; }
  async createContentSchedule(schedule: InsertContentSchedule): Promise<ContentSchedule> { 
    return { id: 1, ...schedule, isActive: schedule.isActive ?? null, lastRunDate: schedule.lastRunDate ?? null, topicCategories: schedule.topicCategories ?? null, publishTime: schedule.publishTime ?? null, createdAt: new Date() }; 
  }
  async updateContentSchedule(id: number, updates: Partial<ContentSchedule>): Promise<ContentSchedule | undefined> { return undefined; }
  async getContentAnalytics(blogPostId: number): Promise<ContentAnalytics | undefined> { return undefined; }
  async createContentAnalytics(analytics: InsertContentAnalytics): Promise<ContentAnalytics> { 
    return { id: 1, ...analytics, blogPostId: analytics.blogPostId ?? null, views: analytics.views ?? null, uniqueViews: analytics.uniqueViews ?? null, timeOnPage: analytics.timeOnPage ?? null, bounceRate: analytics.bounceRate ?? null, socialShares: analytics.socialShares ?? null, emailClicks: analytics.emailClicks ?? null, contactFormSubmissions: analytics.contactFormSubmissions ?? null, lastUpdated: new Date() }; 
  }
  async updateContentAnalytics(id: number, updates: Partial<ContentAnalytics>): Promise<ContentAnalytics | undefined> { return undefined; }
  async getEmailNotifications(): Promise<EmailNotification[]> { return []; }
  async createEmailNotification(notification: InsertEmailNotification): Promise<EmailNotification> { 
    return { id: 1, ...notification, isActive: notification.isActive ?? null, subscribedAt: new Date(), lastSentAt: null }; 
  }
  async updateEmailNotification(id: number, updates: Partial<EmailNotification>): Promise<EmailNotification | undefined> { return undefined; }
  async getContentBackups(blogPostId: number): Promise<ContentBackup[]> { return []; }
  async createContentBackup(backup: InsertContentBackup): Promise<ContentBackup> { 
    return { id: 1, ...backup, blogPostId: backup.blogPostId ?? null, autoGenerated: backup.autoGenerated ?? null, seoScore: backup.seoScore ?? null, changeReason: backup.changeReason ?? null, backupCreatedAt: new Date() }; 
  }
...
export class DatabaseStorage implements IStorage {
  ...
  // Content backup and versioning methods
  async getContentBackups(blogPostId: number): Promise<ContentBackup[]> {
    return await db.select().from(contentBackups)
      .where(eq(contentBackups.blogPostId, blogPostId))
      .orderBy(desc(contentBackups.version));
  }

  async createContentBackup(insertBackup: InsertContentBackup): Promise<ContentBackup> {
  ...
  async restoreContentVersion(blogPostId: number, version: number): Promise<BlogPost | undefined> {
    // Get the backup version
    const [backup] = await db.select().from(contentBackups)
      .where(and(
        eq(contentBackups.blogPostId, blogPostId),
        eq(contentBackups.version, version)
      ));
    
    if (!backup) return undefined;
    
    // Create a new backup of current version before restoring
    const currentPost = await this.getBlogPost(`post-${blogPostId}`);
    if (currentPost) {
      const latestVersion = await this.getLatestVersion(blogPostId);
      await this.createContentBackup({
        ...
        changeReason: 'pre_restore_backup',
        originalPublishedAt: currentPost.publishedAt
      });
    }
    
    // Restore the blog post from backup
    const [restoredPost] = await db
      ...
      .set({
        ...
        seoScore: backup.seoScore
      })
      .where(eq(blogPosts.id, blogPostId))
      .returning();
    
    return restoredPost || undefined;
  }

  async getLatestVersion(blogPostId: number): Promise<number> {
    const [latestBackup] = await db.select({ version: contentBackups.version })
      .from(contentBackups)
      .where(eq(contentBackups.blogPostId, blogPostId))
      .orderBy(desc(contentBackups.version))
      .limit(1);
